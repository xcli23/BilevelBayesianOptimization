...
class UntargetedClassificationDiff(ClassificationGoalFunction):
    ...
    def _get_score(self, model_output, _):
        ...
        if (model_output.numel() == 1) and isinstance(self.ground_truth_output, float):
            ...
            output_else_max = torch.max(torch.cat([model_output[:self.ground_truth_output], model_output[self.ground_truth_output+1:]]))
            return output_else_max - model_output[self.ground_truth_output]
