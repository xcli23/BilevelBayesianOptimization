...
class Attacker:
    ...
    def _attack(self):
        ...
        skipped_idx = []
        my_dict = {}
        pbar = tqdm.tqdm(total=len(worklist), smoothing=0, dynamic_ncols=True)

        kernellist = self.attack_args.kernel.split('-')
        glo.set_value('kernellist',kernellist)
        kernel_num = len(kernellist)
        max_iter=self.attack_args.max_iter
        count_array = [0] * (max_iter + kernel_num - 1)
        threshold = self.attack_args.threshold
        filename = fr'{self.attack_args.pkl_dir}.txt'
        ...
        while worklist:
            ...
            try:
                ...
                try:
                    ...
                    if self.attack.search_method.__class__.__name__ == 'ParticleSwarmOptimization' or self.attack.search_method.__class__.__name__ == 'GreedyWordSwapWIR':
                        from textattack.search_methods.block_bayes_attack import BlackBoxModel
                        self.attack.goal_function.num_queries = 0
                        BBM = BlackBoxModel(model=None, goal_function=self.attack.goal_function, transformer=self.attack.get_transformations)
                        BBM.set_x(example)
                        n_vertices = BBM.n_vertices
                        query_budget = (sum(n_vertices) - len(n_vertices)) * 1     
                        self.attack.goal_function.query_budget = query_budget
                        if query_budget == 0:   
                            self.attack.goal_function.query_budget = 1  
                    ...
                    if kernel_num == 2:
                        ...
                        def attack_function(params):
                            ...
                            BO_query += result.num_queries
                            result_value = result.perturbed_result.raw_output[ground_truth_output]
                            with open(filename, 'a') as file:
                                if iteration < (kernel_num + 2):
                                    score_array[iteration - 1] = result_value
                                    # print(f'score_array:{score_array}')
                                    if all(score > threshold for score in score_array):
                                        print(f"attac-->{idx}, fail",file=file)
                                        raise TypeError
                                if isinstance(last_BO_result, SkippedAttackResult):
                                    print(f"attac-->{idx}, skip",file=file)
                                    raise TypeError
                                if isinstance(last_BO_result, SuccessfulAttackResult):
                                    print(f"attac-->{idx}, succ",file=file)
                                    count_array [iteration - 1] += 1
                                    raise TypeError
                                if isinstance(last_BO_result, FailedAttackResult):
                                    print(f"attac-->{idx}, fail",file=file)
                    ...
                    if kernel_num >= 3: 
                        ...
                        if max_iter == 1:
                            initial_point = np.array([[0.333, 0.5],])
            ...
            pbar.set_description(
                f"[Succeeded / Failed / Skipped / Total / Queries] {num_successes} / {num_failures} / {num_skipped} / {num_results} / {result.num_queries}"
            )
            if idx in successes_idx:
                my_dict[idx] = result.num_queries
                print(f'query:{my_dict}')
        ...
        attack_query_stats = AttackQueries().calculate(self.attack_log_manager.results)
        words_perturbed_stats = WordsPerturbed().calculate(self.attack_log_manager.results)
        success_rate = (len(successes_idx) / (len(successes_idx) + len(failed_ids))) * 100
        with open(filename, 'a') as file:
            output = io.StringIO()
            with contextlib.redirect_stdout(output):
                print(f'count_array:{count_array}')
                print(f'attack_success_rate : {success_rate:.2f}%')
                print(f'avg_word_perturbed_perc : {words_perturbed_stats["avg_word_perturbed_perc"]}%')
                print(f'avg_num_queries : {attack_query_stats["avg_num_queries"]}')
...
def attack_from_queue(
    ...
    # print(f'torch.multiprocessing.current_process():{torch.multiprocessing.current_process()}')
    # print(f'torch.multiprocessing.current_process()._identity[0]:{torch.multiprocessing.current_process()._identity[0]}')
    pid = os.getpid()
    gpu_id = (torch.multiprocessing.current_process()._identity[0] - 1) % num_gpus
    # os.system(f"taskset -cp {gpu_id} {pid}")
    # process_nos = ','.join(str(gpu_id + 4*i) for i in range(10))
    process_nos = f'{(torch.multiprocessing.current_process()._identity[0])*40}-{(torch.multiprocessing.current_process()._identity[0]+1)*44 - 1}'
    # print(process_nos)
    # print(f"taskset -c {process_nos} -p {pid}")
    # os.system(f"taskset -c {process_nos} -p {pid}")
    os.system(f"taskset -cp {process_nos} {pid}")
