...
import torch
import time
def get_query_budget(x, syndict, Dataset, baseline='textfooler'):
    ...
    if Dataset == 'IPS':
        query_budget = int(query_budget /10)
    return int(query_budget)
...
def bayesian_attack(x, y, syndict, BBM, dpp_type='dpp_posterior', block_size=40, max_loop=5, max_patience=20, post_opt='v3', fit_iter=3, Dataset = None):
    '''
        x : size 1 x L tensor
        y : size 1 tensor
    '''
    ...
    if BBM.get_score(x_, require_transform=False) >= 0:
        ...
        return copy.deepcopy(x), None, None, -1, elapsed_time, None, None
    # Success or Fail
    else:
        query_budget = get_query_budget(x_, syndict, Dataset, baseline='textfooler')
        ...
        score, y_prob = BBM.get_score_prob(x_att_transformed,require_transform=False)
        
        succ = 1 if score >= 0 else 0 # 1 if Success else 0.
        elapsed_time = time.time() - init_time
        return x_att_transformed.cuda(), num_queries, modif_rate, succ, elapsed_time, attack_logs, y_prob