...
def fit_model_partial(
    ...
    surrogate_model, train_X = get_data_and_model_partial(hb, opt_indices, init_ind, prev_indices, BBM, embed)
    if params:
        surrogate_model.load_state_dict(params)
    
    optimizer = torch.optim.Adam([
    {'params': [p for n, p in surrogate_model.named_parameters() if n not in['covar_module.weight']]}, 
    ], lr=0.1) 
...
def get_data_and_model_partial(hb, opt_indices, init_ind, prev_indices, BBM, embed):
    ...
    surrogate_model = NewMixedSingleTaskGP(train_X=train_X, train_Y=train_Y, cat_dims=list(range(L)), BBM=BBM, embed=embed).cuda()
