...
class BlockBayesAttack:
    ...
    def perform_search(self, x, n_vertices, BBM):
        ...
        self.eff_len = self.hb.eff_len_seq
        self.BBM = BBM
        self.embed =  hub.load("https://tfhub.dev/google/universal-sentence-encoder/3")   # USEv3
        # self.embed =  hub.load("https://tfhub.dev/google/universal-sentence-encoder-large/5")   # USEv5
        ...
        while self.BLOCK_QUEUE:
            ...
            while stage_call < stage_iter and iter_patience:
                ...
                if prev_size == len(self.hb.eval_Y):
                    ...
                    iter_patience = 5
                    prev_size = len(self.hb.eval_Y)
                self.surrogate_model.fit_partial(self.hb, list(range(self.eff_len)), stage_init_ind, prev_indices=parent_history, BBM=self.BBM, embed=self.embed) 
    ...
    def update_queue(self, Q, I):
        ...
        if len(Q)==1:
            return Q
        self.fit_surrogate_model_by_block_history() 
        if self.surrogate_model.model.covar_module.base_kernel is not None:
    ...
    def final_exploitation_v3(self, seq, ind, forced_inds=[]):
        ...
        while True:
            ...
            self.surrogate_model.fit_partial(self.hb, whole_indices, init_idx, sum_history, self.BBM, self.embed)
    ...
    def fit_surrogate_model_by_block_history(self, forced_inds=[]):
        ...
        self.surrogate_model.fit_partial(self.hb, whole_indices, len(self.hb.eval_Y), sum_history, self.BBM, self.embed)
        return bhl, sum_history
